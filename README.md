...
`prompts/` フォルダ内のテキストファイルを編集することで、生成AIへの指示を自由に調整できます。各ファイルでは以下のパラメータ（プレースホルダー）が使用可能です。

### 1. 構成案生成 (`prompts/outline/`)

| ファイル | 使用可能なパラメータ | 説明 |
| :--- | :--- | :--- |
| `draft.txt` | `{theme}` | 入力ファイル（theme.txt）の内容。 |
| `review.txt` | `{draft}` | 生成された構成案の草案。 |
| `finalize.txt` | `{draft}`, `{review}` | 草案と、それに対するレビュー内容。 |

### 2. 台本生成 (`prompts/script/`)

| ファイル | 使用可能なパラメータ | 説明 |
| :--- | :--- | :--- |
| `draft.txt` | `{title}`, `{description}`, `{context}` | セクションの題名、概要、および**直前のセクションの台本**。 |
| `review.txt` | `{draft}` | 執筆された台本の草案。 |
| `finalize.txt` | `{draft}`, `{review}` | 台本の草案と、それに対するレビュー内容。 |

> [!TIP]
> 台本生成の `draft.txt` では、`{context}` プレースホルダーを使用して前のセクションの内容をプロンプト内の任意の場所に挿入できます。これにより、ストーリーの繋がりをより緻密に制御することが可能です。
